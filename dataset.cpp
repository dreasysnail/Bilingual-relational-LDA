...
int dataset::read_wordmap(string wordmapfile, mapword2id * pword2id) {
	...
	pword2id->insert(pair<string, int>(strtok.token(0), stoi(strtok.token(1).c_str())));
    }
...
int dataset::read_wordmap(string wordmapfile, mapid2word * pid2word) {
	...
	pid2word->insert(pair<int, string>(stoi(strtok.token(1).c_str()), strtok.token(0)));
    }
...
int dataset::read_trndata(string dfile, string wordmapfile) {
	...
	if (length <= 0) {
	    printf("Invalid (empty) document on line %d!\n",i);
	    deallocate();
	    M = V = 0;
	    return 1;
	}
