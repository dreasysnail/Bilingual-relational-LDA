...
int model::load_model(string model_name) {
	...
	for (j = 0; j < length; j++) {
	    ...
	    words.push_back(stoi(tok.token(0).c_str()));
	    topics.push_back(stoi(tok.token(1).c_str()));
	}
...
int model::save_model(string model_name) {
    ...
    if (save_model_beta(dir + model_name + beta_suffix)) {
	return 1;
    }
...
int model::save_model_beta(string filename) {
    FILE * fout = fopen(filename.c_str(), "w");
    if (!fout) {
	printf("Cannot open file %s to save!\n", filename.c_str());
	return 1;
...
int model::save_inf_model(string model_name) {
    ...
    if (save_inf_model_newbeta(dir + model_name + beta_suffix)) {
	return 1;
    }
...
int model::save_inf_model_newbeta(string filename) {
    FILE * fout = fopen(filename.c_str(), "w");
    if (!fout) {
	printf("Cannot open file %s to save!\n", filename.c_str());
	return 1;
...
int model::init_est() {
    //m doc idx; n doc word idx, w all word, k topics
    int m, n, w, k;
    ...
    // alpha, lambda: from command line or default values
...
int model::sampling(int m, int n) {
    ...
    double Vlambda = V * lambda;
    //double Kalpha = K * alpha;
    // do multinomial sampling via cumulative method
    // for each topic , the prob of z[m][n]
    // (ndsum[m] + Kalpha) is useless
    for (int k = 0; k < K; k++) {
        p[k] = (nw[w][k] + lambda) / (nwsum[k] + Vlambda) *
        (nd[m][k] + alpha);
    }
    ...
    // scaled sample because of unnormalized p[]  RAND_MAX=1
...
void model::compute_beta() {
    for (int k = 0; k < K; k++) {
	for (int w = 0; w < V; w++) {
	    beta[k][w] = (nw[w][k] + lambda) / (nwsum[k] + V * lambda);
	}
    }
}
...
int model::init_inf() {
    ...
    newbeta = new double*[K];
    for (k = 0; k < K; k++) {
        newbeta[k] = new double[newV];
    }    
...
int model::inf_sampling(int m, int n) {
    ...
    // do multinomial sampling via cumulative method
    for (int k = 0; k < K; k++) {
	p[k] = (nw[w][k] + newnw[_w][k] + lambda) / (nwsum[k] + newnwsum[k] + Vlambda) *
...
void model::compute_newbeta() {
    map<int, int>::iterator it;
    for (int k = 0; k < K; k++) {
	for (int w = 0; w < newV; w++) {
	    it = pnewdata->_id2id.find(w);
	    if (it != pnewdata->_id2id.end()) {
		newbeta[k][w] = (nw[it->second][k] + newnw[w][k] + lambda) / (nwsum[k] + newnwsum[k] + V * lambda);
	    }
	}
    }
}
